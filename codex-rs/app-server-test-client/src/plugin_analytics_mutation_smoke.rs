...
pub(super) fn run_cleanup(
    ...
    let mut client = SolaiAgentClient::spawn_stdio(codex_bin, &overrides)?;
    client.initialize()?;
...
fn spawn_client(
    ...
    SolaiAgentClient::spawn_stdio_with_env(codex_bin, &overrides, &environment)
}
...
fn uninstall_remote_plugin(client: &mut SolaiAgentClient, remote_plugin_id: &str) -> Result<()> {
    let request_id = client.request_id();
