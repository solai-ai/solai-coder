...
pub struct Arg0DispatchPaths {
    /// Stable path to the current SolaiAgent executable for child re-execs.
    ///
    ...
    /// instead of the real SolaiAgent.
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that SolaiAgent will proceed successfully even if
...
/// While we want to deploy the SolaiAgent as a single executable for simplicity,
...
/// user to install a separate executable, simplifying the deployment of SolaiAgent
/// CLI.
