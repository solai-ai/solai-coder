...
const OPENAI_BETA_HEADER: &str = "SolaiAgent-Beta";
...
/// Options for building a local SolaiAgent diagnostic report.
///
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary =
                    "npm install -g @solaiecosystem/solai would update a different install".to_string();
...
fn describe_method_with_package_layout(
    method: &str,
    package_layout: Option<&SolaiAgentPackageLayout>,
) -> String {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("solai");
...
fn auth_check(config: &Config) -> DoctorCheck {
    ...
    match load_auth_dot_json(
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no SolaiAgent credentials were found",
        )
        .details(details)
...
fn provider_specific_auth_check(
    ...
    details.push(format!(
        "model provider requires SolaiAgent auth: {requires_openai_auth}"
    ));
    ...
    match provider_env_key {
        ...
        None => Some(
            DoctorCheck::new(
                ...
                "auth",
                CheckStatus::Ok,
                "SolaiAgent auth is not required for the active model provider",
            )
            .details(details),
        ),
    }
...
fn auth_mode_name(auth: &SolaiAgentAuth) -> &'static str {
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/solai");
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/solai");
    ...
    fn provider_specific_auth_allows_non_openai_provider_without_env_key() {
        ...
        .expect("non-SolaiAgent provider should produce a provider-specific check");
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running SolaiAgent."),
            Vec::new(),
            |_| false,
        )
        .expect("non-SolaiAgent provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running SolaiAgent.".to_string())
        );
    }
