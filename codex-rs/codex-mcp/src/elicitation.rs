...
//!
//! RMCP clients call into this module when a server asks SolaiAgent to elicit data
//! from the user. It decides whether the request can be automatically accepted,
//! must be declined by policy, or should be surfaced as a SolaiAgent protocol event
...
/// generated by SolaiAgent rather than copied from the MCP connection, so separate runtimes may reuse
