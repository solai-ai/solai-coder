...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestSolaiAgent) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured SolaiAgent helper binary
...
impl Expectation {
    fn verify(&self, test: &TestSolaiAgent, result: &CommandResult) -> Result<()> {
        match self {
...
fn assert_active_workspace_permission_profile(test: &TestSolaiAgent) {
    assert_eq!(
...
async fn wait_for_spawned_thread(test: &TestSolaiAgent) -> Result<Arc<SolaiAgentThread>> {
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Print an ssh command that recreates the current SolaiAgent sandbox remotely."
    )
...
"#
    ...
    assert_eq!(
        ...
        "remote_bash.py should use the allowlisted inherited profile and managed configuration to reconstruct the SolaiAgent sandbox"
    );
