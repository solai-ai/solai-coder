...
pub struct SolaiAgentToolCallParam {
    /// The *initial user prompt* to start the SolaiAgent conversation.
    pub prompt: String,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub approval_policy: Option<SolaiAgentToolCallApprovalPolicy>,

    /// Sandbox mode: `read-only`, `workspace-write`, or `danger-full-access`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub sandbox: Option<SolaiAgentToolCallSandboxMode>,
...
pub enum SolaiAgentToolCallApprovalPolicy {
    Untrusted,
    OnRequest,
    Never,
}

impl From<SolaiAgentToolCallApprovalPolicy> for AskForApproval {
    fn from(value: SolaiAgentToolCallApprovalPolicy) -> Self {
        match value {
            SolaiAgentToolCallApprovalPolicy::Untrusted => AskForApproval::UnlessTrusted,
            SolaiAgentToolCallApprovalPolicy::OnRequest => AskForApproval::OnRequest,
            SolaiAgentToolCallApprovalPolicy::Never => AskForApproval::Never,
        }
    }
}
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq)]
#[serde(rename_all = "kebab-case")]
pub enum SolaiAgentToolCallSandboxMode {
    ReadOnly,
    WorkspaceWrite,
    DangerFullAccess,
}

impl From<SolaiAgentToolCallSandboxMode> for SandboxMode {
    fn from(value: SolaiAgentToolCallSandboxMode) -> Self {
        match value {
            SolaiAgentToolCallSandboxMode::ReadOnly => SandboxMode::ReadOnly,
            SolaiAgentToolCallSandboxMode::WorkspaceWrite => SandboxMode::WorkspaceWrite,
            SolaiAgentToolCallSandboxMode::DangerFullAccess => SandboxMode::DangerFullAccess,
        }
    }
}

/// Builds a `Tool` definition (JSON schema etc.) for the SolaiAgent tool-call.
pub(crate) fn create_tool_for_codex_tool_call_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        ...
        .into_generator()
        .into_root_schema_for::<SolaiAgentToolCallParam>();

    let input_schema =
        create_tool_input_schema(schema, "SolaiAgent tool schema should serialize");

    Tool::new(
        "codex",
        "Run a SolaiAgent session. Accepts configuration parameters matching the SolaiAgent Config struct.",
        input_schema,
    )
...
impl SolaiAgentToolCallParam {
    /// Returns the initial user prompt to start the SolaiAgent conversation and the
...
pub struct SolaiAgentToolCallReplyParam {
    /// DEPRECATED: use threadId instead.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    conversation_id: Option<String>,

    /// The thread id for this SolaiAgent session.
    ...
    /// The *next user prompt* to continue the SolaiAgent conversation.
    pub prompt: String,
}

impl SolaiAgentToolCallReplyParam {
...
pub(crate) fn create_tool_for_codex_tool_call_reply_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        ...
        .into_generator()
        .into_root_schema_for::<SolaiAgentToolCallReplyParam>();

    let input_schema =
        create_tool_input_schema(schema, "SolaiAgent reply tool schema should serialize");

    Tool::new(
        "codex-reply",
        "Continue a SolaiAgent conversation by providing the thread id and prompt.",
        input_schema,
    )
    .with_title("SolaiAgent Reply")
    .with_raw_output_schema(codex_tool_output_schema())
}
...
mod tests {
    ...
    fn verify_codex_tool_json_schema() {
        ...
        let expected_tool_json = serde_json::json!({
          "description": "Run a SolaiAgent session. Accepts configuration parameters matching the SolaiAgent Config struct.",
          "inputSchema": {
            ...
            "properties": {
              ...
              "prompt": {
                "description": "The *initial user prompt* to start the SolaiAgent conversation.",
                "type": "string"
              },
              "sandbox": {
    ...
    fn codex_tool_call_param_rejects_removed_profile_field() {
        let err = serde_json::from_value::<SolaiAgentToolCallParam>(serde_json::json!({
            "prompt": "hello",
            "profile": "work"
        }))
    ...
    fn verify_codex_tool_reply_json_schema() {
        ...
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Continue a SolaiAgent conversation by providing the thread id and prompt.",
          "inputSchema": {
            "properties": {
              "conversationId": {
              ...
              "prompt": {
                "description": "The *next user prompt* to continue the SolaiAgent conversation.",
                "type": "string"
              },
              "threadId": {
                "description": "The thread id for this SolaiAgent session. This field is required, but we keep it optional here for backward compatibility for clients that still use conversationId.",
                "type": "string"
              }
            },
            "required": [
