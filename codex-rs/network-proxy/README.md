...
`codex-network-proxy` is SOLAI Agent's local network policy enforcement proxy. It runs:
...
`codex-network-proxy` reads from SOLAI Agent's merged `config.toml` (via `codex-core` config loading).
...
When `codex-network-proxy` is embedded in managed SOLAI Agent runtime, policy decisions emit structured
