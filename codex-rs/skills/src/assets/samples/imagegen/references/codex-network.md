# SOLAI Agent network approvals / sandbox notes
...
The fallback CLI uses the SOLAI Agent Image API, so it needs outbound network access. In many SOLAI Agent setups, network access is disabled by default and/or the approval policy requires confirmation before networked commands run.
...
- In `workspace-write`, network access still depends on your SOLAI Agent configuration (for example `[sandbox_workspace_write] network_access = true`).
