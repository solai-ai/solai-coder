...
async fn slash_compact_eagerly_queues_follow_up_before_turn_start() {
    ...
    match rx.try_recv() {
        Ok(AppEvent::SolaiAgentOp(Op::Compact)) => {}
...
async fn queued_slash_compact_dispatches_after_active_turn() {
    ...
    assert!(
        events
            .iter()
            .any(|event| matches!(event, AppEvent::SolaiAgentOp(Op::Compact))),
...
async fn queued_bare_rename_drains_next_input_after_name_update() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::SolaiAgentOp(Op::SetThreadName { name }) if name == "Queued rename"
        )),
...
async fn slash_rename_prefills_existing_thread_name() {
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::SolaiAgentOp(Op::SetThreadName { name })) if name == "Current project title"
    );
}
...
async fn pending_token_activity_refresh_keeps_composer_visible_in_short_viewport() {
    ...
    assert!(
        term.backend()
            ...
            .screen()
            .contents()
            .contains("Ask SolaiAgent to do anything")
    );
}
