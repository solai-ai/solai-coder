...
- `SolaiAgent` is the sync public API.
- `AsyncSolaiAgent` is an async replica of the same public API shape.
- Prefer `async with AsyncSolaiAgent()` for async code. It is the standard path for
  explicit startup/shutdown, and `AsyncSolaiAgent` initializes lazily on context
  entry or first awaited API use.

If your app is not already async, stay with `SolaiAgent`.
...
When `sandbox=` is omitted, SOLAI Agent uses its configured default. A turn
...
`SolaiAgent()` is eager: it starts transport and calls `initialize` in `__init__`.

Common causes:

- installation is incomplete and the pinned `openai-codex-cli-bin` dependency is missing
- local `codex_bin` override points to a missing file
- a custom local SOLAI Agent executable does not support the SDK operation being used
